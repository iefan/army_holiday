...
class FrameMain(wx.Frame):
    def __init__(self, flagAdmin, curUser):
        ...
        menuChildStr = [[u"用户维护",u"",  u"数据导入...", u"数据备份", u"导入备份文件...", u"", u"休假统计", u"", u"退出系统"], \
            [u"批假", u"销假及查询"], \
            [u"单位维护", u"人员信息", u"", u"军衔维护", u"路途维护", u"假期维护"], \
            [u"帮助...", u"", u"关于..."]]
        menuTipStr = [[u"增删、更改用户信息", u"", u"从Excel表格导入数据", u"数据备份",  u"导入备份文件...", u"", u"休假统计", u"",u"退出系统"], \
            [u"对单位人员进行批假登记",  u"查询休假人员及对到假人员进行登记"], \
            [u"对单位维护", u"人员基本数据信息维护",u"", u"对军衔维护", u"对路途维护", u"对法定节假日维护"], \
            [u"获取帮助", u"", u"系统说明"]]
        menuFunLst = [[self.OnUserFix, self.OnImportdata, self.OnExportdata, self.OnImportPHR, self.OnStatPic, self.OnClose],\
...
#        self.sb.SetStatusText(u"研制单位：91960部队71分队", 1)
        self.sb = CustomStatusBar(self)
        self.SetStatusBar(self.sb)
#        self.timer = wx.PyTimer(self.NotifyBar)
#        self.timer.Start(1000)
    ...
    def OnStatPic(self, event):
        iRow = 0; iCol = 1
        self.ShowPanel(iRow, iCol, frmStatPic.StatPic)
    ...
    def OnCloseWindow(self, event):
        ...
        if wx.OK == retv:
            ...
            self.flagExit = 1
            for item in self.panel.GetChildren():
                if item.GetClassName() == u'wxStaticBox':
                    self.flagExit = 0
                    [ich.Destroy() for ich in self.panel.GetChildren()]
                    self.panelDict.clear()
                    break
