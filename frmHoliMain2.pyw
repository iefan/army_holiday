...
#        self.SetBackgroundColour(())
        ...
        menuBar = FM.FlatMenuBar(self, -1, 32, 5, options = FM_OPT_SHOW_TOOLBAR | FM_OPT_SHOW_CUSTOMIZE)
#        menuBar = wx.MenuBar()
        menuStrlst = [u"系统设置(&S)", u"批假销假(&P)", u"信息维护(&M)", u"帮助(&H)"]
        menuToplst = []
        for index in range(len(menuStrlst)):
            menuToplst.append(FM.FlatMenu())
        ...
        for menuT, menuC, menuTip, menuFun in zip(menuToplst, menuChildStr, menuTipStr, menuFunLst):
            ...
            for index in range(len(menuC)):
                if menuC[index] != "":
                    iNum += 1
                    menuChildlst[menuT].append(menuT.AppendItem(FM.FlatMenuItem(menuT, -1, menuC[index], menuTip[index])))
...
#        self.SetMenuBar(menuBar)
        ...
        #===================================================
#        tb = FM.FlatMenuBar(self, -1, 32, 5, options = FM_OPT_SHOW_TOOLBAR | FM_OPT_SHOW_CUSTOMIZE)
#        tb = self.CreateToolBar(TBFLAGS)
        ...
        for index in range(len(imgNamelst)):
            if imgNamelst[index] != "":
                ...
                menuBar.AddTool(tbId,tiptoobar[idcount], icontoobar[idcount], shortHelp=tiptoobar[idcount], longHelp=hlptoobar[idcount])
...
#        self.SetStatusBar(self.sb)
        ...
        mainSizer.Add(menuBar, 0, wx.EXPAND)
        mainSizer.Add(self.panel, 1, wx.EXPAND)
        mainSizer.Add(self.sb, 1, wx.EXPAND)
